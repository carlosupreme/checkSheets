...
@foreach($chunks as $index => $chunk)
    <div @if(!$loop->last) class="page-break" @endif>
        <table>
            <tr>
                <td colspan="7" class="header">{{ $record->equipment_name }}</td>
                <td colspan="8" class="text-right">Página {{ $index + 1 }}</td>
            </tr>
            <tr>
                <td colspan="5" class="subheader">HOJA DE CHEQUEO DE EQUIPO</td>
                <td colspan="5" class="subheader">AREA: {{ $record->equipment_area }}</td>
                <td colspan="5" class="subheader">TAG: {{ $record->equipment_tag }}</td>
            </tr>
        </table>
                        ...
                        <td class="date-column">
                            @if(isset($tableData['checks'][$day][$index]))
                                @if($tableData['checks'][$day][$index]['icon'])
                                    <span style="color: {{$tableData['checks'][$day][$index]['color']}}">
                                      @php
                                          preg_match('/heroicon-(?<s>[oscm])-([a-z\-]+)/', $tableData['checks'][$day][$index]['icon'], $matches);
                                           if ($matches) {
                                              $type = $matches['s'];
                                              $iconName = $matches[2];
                                              if ($type === 'o') {
                                                  echo '<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6"><use href="#' . $iconName . '"></use></svg>';
                                               }
                                                else if ($type === 's') {
                                                     echo '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" class="w-6 h-6"><use href="#' . $iconName . '"></use></svg>';
                                                }
                                                else if ($type === 'c') {
                                                  echo '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" class="w-6 h-6"><use href="#' . $iconName . '"></use></svg>';
                                                }
                                              else if ($type === 'm') {
                                                     echo '<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-4 h-4"><use href="#' . $iconName . '"></use></svg>';
                                               }
                                           }
                                      @endphp
                                    </span>
